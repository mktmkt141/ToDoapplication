...
* cookieによる認証・・・本アプリケーションでは、ユーザー認証にhttpOnly属性を持つCookieを利用したセッション管理方式を採用しました。以前の開発ではJWTをlocalStorageに保存する方式を採用しましたが、今回はセキュリティとシンプルさを重視し、以下の理由からCookieセッション方式を選択しました。<br>
